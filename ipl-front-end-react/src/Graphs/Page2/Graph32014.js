...
class Graph extends Component {
  constructor(iplData) {
    ...
    // const iplYear = iplData.props.map((item) => item.year == 2014);
    let iplExtRun = iplData.props.filter((item) => item.year == "2014");
    iplExtRun = iplExtRun.map((item) => item.extra_run);
    let iplTeams = iplData.props.filter((item) => item.year == "2014");
