...
class Graph extends Component {
  constructor(iplData) {
    ...
    // const iplYear = iplData.props.map((item) => item.year == 2012);
    let iplPlayed = iplData.props.filter((item) => item.year == "2012");
    iplPlayed = iplPlayed.map((item) => item.matches_played);
    let iplTeams = iplData.props.filter((item) => item.year == "2012");
